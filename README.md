...
### ccs (le TUI)
- Colonnes **PROJECT / STATUS / AGENT / HOST / TITLE / … / LAST PROMPT**. STATUS thémé (jacked in / in construct / awaiting / unplugged). HOST = kitty / vscode / bg.
...
### kitty
- Fenêtres agent + **fenêtre « The Matrix »** (ccs, thème vert `matrix-window.conf` + fond `matrix-bg.png`). Monitoring k9s (`sessions/*.session`). Actions clic-droit (`kitty.desktop`) : **Red Pill**, **The Matrix**, **Monitoring local/develop/prod**.
...
scripts/
  ...
  claude-sessions   ccs : le TUI (colonnes, focus ⏎, bouton Red Pill, thème Matrix)
hooks/
...
- **Lancer un agent** : clic-droit kitty → **Red Pill** (menu : reprendre une session récente **ou** nouvelle → choix du repo), ou `redpill` / `matrix` dans un terminal, ou le **bouton `[⏏ Red Pill]`** dans ccs.
